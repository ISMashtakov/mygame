...
func (c SimpeSpriteCreator) Create(
	world donburi.World,
...
func (c SimpeSpriteCreator) DivideSprite(
	world donburi.World,
	sprite components.SpriteData,
	pos components.PositionData,
) []*donburi.Entry {
	getSprite := func(imageRect image.Rectangle, shiftX, shiftY float64) *donburi.Entry {
		ent := c.Create(world, components.SpriteData{Z: sprite.Z, Image: images.Image{
			...
			Vec: pos.Add(
				gmath.VecFromStd(sprite.Image.Bounds().Size()).
					Mul(sprite.Image.Scale).
					Mulf(0.25).
					Mul(gmath.Vec{X: shiftX, Y: shiftY}),
			),
		})
	...
	if sprite.Image.Flip {
		lu := getSprite(
			image.Rect(sprite.Image.Bounds().Dx()/2, 0, sprite.Image.Bounds().Dx(), sprite.Image.Bounds().Dy()/2),
			-1,
			-1,
		)
		ru := getSprite(image.Rect(0, 0, sprite.Image.Bounds().Dx()/2, sprite.Image.Bounds().Dy()/2), 1, -1)
		rd := getSprite(
			image.Rect(0, sprite.Image.Bounds().Dy()/2, sprite.Image.Bounds().Dx()/2, sprite.Image.Bounds().Dy()),
			1,
			1,
		)
		ld := getSprite(
			image.Rect(
				sprite.Image.Bounds().Dx()/2,
				sprite.Image.Bounds().Dy()/2,
				sprite.Image.Bounds().Dx(),
				sprite.Image.Bounds().Dy(),
			),
			-1,
			1,
		)
